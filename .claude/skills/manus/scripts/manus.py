...
def stream_task(task_id):
    ...
    shown = 0
    time.sleep(2)  # let Manus initialize the task
    while True:
        try:
            r = requests.get(f"{BASE_URL}/tasks/{task_id}", headers=headers())
            if r.status_code in (404, 429, 500, 502, 503):
                time.sleep(3); continue  # task not ready yet, keep waiting
...
def task_create(prompt, mode="agent", model="manus-1.6", attachments=None, task_id=None):
...
def cmd_send(args):
    ...
    resp = task_create(args.prompt, args.mode, model, attachments or None, task_id=thread)
    if not resp: return
...
def cmd_chat(args):
    ...
    last_task_id = None  # reuse thread for follow-up messages
    ...
    while True:
        ...
        task = task_create(prompt, mode, model, attachments, task_id=last_task_id)
...
{BOLD}Local task:{RESET}
  manus.py local "<task>"              Send task to Manus agent
""")
